...
from gemeinsam import BASE, DATA, SITE
...
def test_die_quellen_der_fussnote_stimmen_mit_dem_scraper_ueberein():
    ...
    """
    quellen = (BASE / "scraper" / "quellen.py").read_text(encoding="utf-8")
    m = re.search(r"^QUELLEN = \[(.*?)^\]", quellen, re.S | re.M)
    assert m, "QUELLEN-Liste nicht gefunden"
    namen = re.findall(r'Quelle\("(\w+)"', m.group(1))
