...
if __name__ == "__main__":     # Abdeckung gegen den echten Datenbestand pruefen
    ...
    for f in festivals:
        ...
        for angabe in text.split(","):
            ...
            if angabe:
                haeufig[angabe] += 1
                if not oberbegriffe(angabe):
                    offen[angabe] += 1
        keys = oberbegriffe(text)
        if keys:
            mit += 1
            for k in keys:
                je_festival[k] += 1
    ...
    gesamt = sum(haeufig.values())
    print(f"Angaben {gesamt} in {len(haeufig)} Schreibweisen, "
          f"nicht zugeordnet: {sum(offen.values())} "
          f"({sum(offen.values()) / gesamt:.1%}) in {len(offen)} Schreibweisen")
    print(f"Festivals mit Oberbegriff {mit}, mit Genretext ohne Treffer {ohne}, "
          f"ohne Genretext {len(festivals) - mit - ohne}")
    for k, name in OBERBEGRIFFE.items():
        print(f"  {je_festival[k]:5d}  {name}")
    print("\nHaeufigste nicht zugeordnete Angaben:")
    for k, v in offen.most_common(60):
        print(f"  {v:4d}  {k}")
